#!/usr/bin/python3
"""Defines an object to JSON"""
import json


def save_to_json_file(my_obj, filename):
    """
    Writes an object to a text file using its JSON representation.
    Args:
        my_obj: The object to be saved to the file.
        filename: The name of the file to write the JSON representation to.
    Note:
        This implementation uses the `json` module for JSON serialization.
        If the object cannot be serialized, no exception handling is performed.
        File permission exceptions are not managed.
    """
    with open(filename, "w") as file:
        json.dump(my_obj, file)
